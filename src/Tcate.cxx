...
//*************************************************************************
Tcate::Tcate(std::string name, std::string name_of_silicon_at_target ): TNeedle_element(name)
{
  ...
  //  std::cout << "construcor of " << name_of_this_element << std::endl;
...
void Tcate::create_my_spectra()
{
  ...
  std::string folder = "cate_folder/"  ;
  std::string name =   name_of_this_element + "_position_xy" ;
...
//                790, -79, 79,     // -0.5, 0.5,
  ...
  name =  name_of_this_element +
    "_dE_vs_E_gated_cristal_multiplicityA" ;

  matr_total_dE_E_gated_by_cristal_multiplicityA
  ...
  give_spectra_pointer()->push_back(matr_total_dE_E_gated_by_cristal_multiplicityA) ;



  name =   name_of_this_element +
    "_dE_vs_E_gated_cristal_multiplicityB" ;

  matr_total_dE_E_gated_by_cristal_multiplicityB
     = new spectrum_2D( name,
                name,
                100, 1, 4096,
                100, 1, 4096,
  ...
  give_spectra_pointer()->push_back(matr_total_dE_E_gated_by_cristal_multiplicityB) ;


  name =  name_of_this_element +
    "_dE_vs_E_gated_cristal_multiplicityC" ;

  matr_total_dE_E_gated_by_cristal_multiplicityC
     = new spectrum_2D( name,
                name,
                100, 1, 4096,
                100, 1, 4096,
  ...
  give_spectra_pointer()->push_back(matr_total_dE_E_gated_by_cristal_multiplicityC) ;
...
//  total_Si_time_gate_high = 4095;
  ...
  name =   name_of_this_element +
    "_seg_5_dE_vs_cristal_multiplicity" ;
...
//*************************************************************************
void Tcate::make_preloop_action(std::ifstream &)
{
  // we do not use the stream here, because Cate has its own calibration file.

  //std::cout << "Tcate::make_preloop_action "  << std::endl;

  // Reading the distance of the Cate detector
  std::string geometry_file = path.dir_calibration() + "cate_geometry.txt" ;
  ...
  try{
    ...
    if(! plik_geometry)
   {
     std::string mess = "I can't open  file: " + geometry_file  ;
     throw mess ;
   }
    std::string slowo = name_of_this_element + "_distance";
...
//      std::cout << i << ") Address of cate segment " << & (event_unpacked->cate_detector[i]) << std::endl;
...
//      k_ptr = &event_unpacked->cate_detector[i] ;
  ...
  std::cout << "End of Tcate preloop " << std::endl;
...
void Tcate::read_calibration_and_gates_from_disk()
{
...
#ifdef CATE_PRESENT
  std::cout << "\n\n\n\nReading Cate calibration from the disk" << std::endl ;
  ...
  std::string cal_filename = path.dir_calibration() + "cate_calibration.txt";
  try{
    std::ifstream plik(cal_filename.c_str() ) ; // ascii file
    if(!plik)
      {
     std::string mess = "I can't open cate calibration file: "
    + cal_filename  ;
     throw mess ;
      }
    ...
    for(unsigned int i = 0 ; i < telescope.size() ; i++)
      {
     std::cout << "reading calibration loop for cate segment" << i << std::endl ;
  ...
  std::cout << "cate calibration successfully read"
       << std::endl;
  ...
  std::cout << " reading the gates for " << name_of_this_element << "  " << std::endl ;
...
//
//  std::string nam_gate = std::string(path.dir_gates() + ")
//    + name_of_this_element
//    + "_total_CsI_time_cal"
//    + "_gate_GATE.gate" ;
//
//  std::ifstream plik_gates(nam_gate.c_str() );
//  if(! plik_gates)
//    {
//   std::string mess = "I can't open  file: " + nam_gate  ;
//   // throw mess ;
//   std::cout << mess << "\nSo I assume that the gate is wide open !!!!!!!!!!! \a\a\a\n" << std::endl;
//   std::ofstream pliknew(nam_gate.c_str() );
//   pliknew <<  total_CsI_time_gate_low
//        << "   "
//        << total_CsI_time_gate_high
//        << "\t\t // left and right value for the gate" << std::endl;
//
//    }else{
//
//
...
//   if(! plik_gates)
//     {
//       std::string mess = "Error while reading 2 numbers from the gate file: " + nam_gate  ;
//       exit(-1) ;
//     }
//   plik_gates.close() ;
//    }
...
//  nam_gate = std::string(path.dir_gates() + ")
//    + name_of_this_element
//    + "_total_Si_time_cal"
//    + "_gate_GATE.gate" ;
//
//  std::ifstream plik_gates2(nam_gate.c_str() );
//  if(! plik_gates2)
//    {
//   std::string mess = "I can't open  file: " + nam_gate  ;
//   // throw mess ;
...
//std::cout << "Cate analyse current event" << std::endl;
...
// ask Rady for details!!!
  ...
  //      std::cout << "Cate segment " << i+1 << " fired " << std::endl;
      id_of_fired = i+1 ; // start counting from 1-9
      id_of_Si_fired_when_multiplicity_1 = i ; 
  
      spec_fan->increment((int)(i+1));  // they like (1-9)
      ...
      if(flag_exact_xy_ok)
      {
        ...
        matr_position_xy->increment(x_position, y_position);

       x_position_corr = telescope[i]->give_x_corr();
        y_position_corr = telescope[i]->give_y_corr();

        matr_position_xy_corr->increment(x_position_corr, y_position_corr);
...
//        {
//          std::cout << "Exact position x = "
//              << x_position
//              << "y =" << y_position << std::endl;
//
//        }
      }
//     else
...
////        std::cout << "No Exact position, only center available  x = "
////            << x_position
////            << "y =" << y_position << std::endl;
//
//
//      }
  ...
  spec_Si_multiplicity->increment(multiplicity_of_Si_hits);
  spec_CsI_multiplicity->increment(multiplicity_of_CsI_hits);

  // WARNING:   now it is inside the for loop. When we require only one
  // hit - it should (?) be outside

  // here we cold make some total spectra? or let the cristals do it
  if(multiplicity_of_Si_hits==1 && multiplicity_of_CsI_hits==1)
  {
    flag_only_one_hit = true ;
    //     std::cout << "cate: There was one hit " << std::endl;
     ...
     {
               std::cout  << name_of_this_element << ":  (Precisely) ONE segment fired " << std::endl;
     }

      // cate -----------------

//        TRisingAnalysis::fxCaliEvent->cate_dE = dE ;
//        TRisingAnalysis::fxCaliEvent->cate_E   = E;

 

#ifdef CATE_PRESENT
     TRisingAnalysis::fxCaliEvent->cate_x_pos = x_position_corr;
     TRisingAnalysis::fxCaliEvent->cate_y_pos = y_position_corr;

     // putting this below is
     TRisingAnalysis::fxCaliEvent->cate_segment_which_fired_nr
  ...
  {
     // std::cout << "There was MORE than one hit " << std::endl;
...
//  {
//    std::cout << "Cate dE E zero ,when flag  "
...
{
  // std::cout << "F.Tcate::make_postloop_action() " << std::endl ;

  // save all spectra made in Cate and in telescopes
  save_spectra();


  // std::cout << "end of F.Tcate::make_postloop_action() " << std::endl ;
}
//***********************************************************************
...
void Tcate::save_spectra()
{
  ...
  for(unsigned int i = 0 ; i < cate_spectra_list.size() ; i++)
    {
      //std::cout << "Trying to save spectrum nr " <<  i << std::endl ;
...
//    if(nr == 1)
//    {
//      matr_total_dE_E_gated_by_cristal_multiplicityA->increment(E[0], dE[0]);
//    }
//    if(nr == 2)
//    {
//      matr_total_dE_E_gated_by_cristal_multiplicityB->increment(E[0], dE[0]);
//    }
//    if(nr == 3)
//    {
//      matr_total_dE_E_gated_by_cristal_multiplicityC->increment(E[0], dE[0]);
//    }
//  }
//  else if( dE.size() > 1)
//  {
//    //std::cout << "Tcate Many hits " << dE.size() << std::endl;
//  }
//}
...
#else
  std::cout << "This function Tcate::give_central_x_pos_of_segment_which_fired() should be never called "
  << std::endl;
  return 0;
#endif
  }
