/***************************************************************************
                          Ttarget_galileo.cpp  -  description
                             -------------------
...
#include "TEAGLE_crystal.h"   // for sending the doppler options
...
//************************************************************************
Ttarget_galileo::Ttarget_galileo(std::string name_of_target,
                                 std::string name_focus_before,
                                 std::string name_cate_after
                                 ) : TNeedle_element(name_of_target)
{

    focus_before = nullptr;
    ...
    // difference from above - grebware options (how to track mw or not)
...
//***************************************************************************
Ttarget_galileo::~Ttarget_galileo()
{}
//***************************************************************************
/** spectra of angles - theta, phi projectile/scattered_particle */
void Ttarget_galileo::create_my_spectra()
{
...
//**************************************************************************
void  Ttarget_galileo::make_preloop_action(std::ifstream & s )
{
...
//    cout << __PRETTY_FUNCTION__ << endl;
    ...
    try
    {
        ...
        tracking_position_on_target =
                ...
                Tfile_helper::find_in_file<int> (plik,"projectile_tracking");

        tracking_position_on_cate =
                (type_of_cate_requirements)
                Tfile_helper::find_in_file<int>(plik,"lycca_tracking");

        which_doppler_algorithm =
                (type_of_doppler_algorithm)
                Tfile_helper::find_in_file<int>(plik,"doppler_corr_algorithm");


        beta_from_where =  (type_of_beta) Tfile_helper::find_in_file<int>(plik, "beta_from_where");
        beta_fixed_value =  Tfile_helper::find_in_file<double>(plik, "beta_fixed_value");
        energy_deposit_in_target
                =  Tfile_helper::find_in_file<double> (plik, "energy_deposit_in_target");
    ...
    //    TEAGLE_array::set_doppler_corr_options(
    //        tracking_position_on_target,
    //        tracking_position_on_cate,
    //        which_doppler_algorithm);
...
//**********************************************************************
void Ttarget_galileo::analyse_current_event()
{

    //std::cout << "make_calculation for " <<  name_of_this_element << std::endl ;
    ...
    //    TGalileoAnalysis::fxCaliEvent->target_x_pos = 0.0;//x_tracked;
    //    TGalileoAnalysis::fxCaliEvent->target_y_pos = 0.0;//y_tracked;
...
double Ttarget_galileo::give_beta_after_target()
{
...
void Ttarget_galileo::analysis_for_basic_algorithm()
{
    flag_doppler_possible = true ;  // (only by theta of the crystal)

    // just drawing the matrix
    ...
    //        matr_position_xy->increment(x_extrapolated, y_extrapolated);
...
//***********************************************************************
/** No descriptions */
Tcate * Ttarget_galileo::give_cate_ptr()
{
    return cate_after ;
}
/************************************************************************/
void Ttarget_galileo::make_xy_on_cate_distance()
{
    ...
    //        matr_xy_projected_on_cate->increment(x_on_cate, y_on_cate);
    //    }
}
//***************************************************************************
/** to test the extrapolation algorithm */
//void Ttarget_galileo::make_xy_on_any_distance(double dist)
//{
//
//	if(focus_before->was_x_ok() && focus_before->was_y_ok())
//	{
...
//*************************************************************************************************
void Ttarget_galileo::give_scattered_particle_vector(double *x, double *y,
...
void Ttarget_galileo::analysis_with_TVector3()
{
...
//****************************************************************************
/** No descriptions */
void Ttarget_galileo::read_beta_playfield()
{
    ...
    try
    {
        ...
        beta_playfield_enabled =
                (bool) Tfile_helper::find_in_file<bool>(plik,"beta_playfield_enabled");
        ...
        for(int i = 0 ; i < 9 ; i++)
        {
            ...
            betaN_fixed_value[i] =  (double) Tfile_helper::find_in_file<double> (plik, keyw);
    ...
    {
        std::cout << "Ttarget_galileo:: Error while reading the file " << fname
                  << m.message << std::endl;
        throw ;
    }
}
