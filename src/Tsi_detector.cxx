...
//************************************************************************
Tsi_detector:: Tsi_detector(std::string name,
                            cate_segment (TGalileoEvent::*ptr))
    :
    TNeedle_element(name),
...
void Tsi_detector::create_my_spectra()
{
  ...
  std::string folder = "target_folder/" + name_of_this_element ;
  std::string name ;
...
//**************************************************************
//std::vector< spectrum_abstr*>*  Tsi_detector::frs_spectra_ptr
//{
...
void Tsi_detector::analyse_current_event()
{
  ...
  spec_Si_energy->increment( Si_energy_raw ) ;
  // incrementing the raw time spectrum
  spec_Si_time->increment(Si_time_raw);
  ...
  for(int i = 0 ;  i < 4 ;i++)
  {
    spec_pin_raw[i]->increment(pin_raw[i] )  ;
  ...
  if(flag_all_pins_in_gates)
  {
    ...
    if(mianownik !=0)
    {
      ...
      //       spec_x_corr->increment(x_corr) ;
      y_corr = (y_position_corr_factors[1] * y) + y_position_corr_factors[0]  ;
      //        spec_y_corr->increment(y_corr) ;

      flag_xy_ok = true ;

      //std::cout << " incrementing the matrix x,y = " << x << ", " << y << std::endl ;
      matr_position_xy->increment(x, y);
      matr_position_xy_corr->increment(x_corr, y_corr);
    }
  }
...
void Tsi_detector::make_preloop_action(std::ifstream & s)
{
  //
  //  std::cout << "preloop action for si_detector "
  //       << name_of_this_element
  //       << std::endl ;



  // std::cout << "Reading the calibration for the si_detector "
  //        << name_of_this_element
  //        << std::endl ;

  //  cate_segment (TGalileoEvent::*ptr_to_si);
  //
  ...
  //  std::string name_of_pin; // [100] = "" ;
  for(int symb = 0 ; symb < 4 ; symb++)
  {
    std::ostringstream outs;  // Declare an output std::string stream.
    outs << name_of_this_element << "_pin_"
    << char('a' + symb )
    << "_cal_factors";
    //<< ends ;   // Convert value into a string.   << this was giving an error !!!!!!!!!!!!!!!!!!!!!!!!!!!!

    slowo = outs.str();      // Get the created std::string from the output stream.

    //std::cout << "Try to spot the word >>" << slowo << "<<" << std::endl;
    Tfile_helper::spot_in_file(s, slowo );
...
void Tsi_detector::read_gates()
{
  ...
  for(int symb = 0 ; symb < 4 ; symb++)
  {
    ...
    else
    {
      ...
      if(! plik_gate)
      {
        std::cout <<  "Error while reading 2 numbers from the gate file: " + nam_gate  ;
        exit(-1) ;
      }
      plik_gate.close() ;
    }
