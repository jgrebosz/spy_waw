...
using namespace std;
//**************************************************************************
Tlookup_table_doublets::Tlookup_table_doublets(uint max_first, uint max_second, string pref)
{
    max_group = max_first;
...
//  string name = "./mbs_settings/lookup_table.txt" ;
    ...
    if(!plik)
    {
        cout << "!!! Fatal error: I can't open lookup_table file: " << name << endl;
        exit(1);
    }
    ...
    try
    {
        ...
        for(unsigned int clus = 0 ; clus < marks.size() ; clus++)
        {
            for(int segm = 0 ; segm < 7 ; segm++)
            {
                ...
                unsigned int group =
                     Tfile_helper::find_in_file<unsigned int>(plik, det_name) ;
