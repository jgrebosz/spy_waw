...
//************************************************************************
Tcate_telescope:: Tcate_telescope(Tcate *ptr_owner, std::string name, int id_nr)
  : owner(ptr_owner),
...
void Tcate_telescope::create_my_spectra()
{
  ...
  std::string folder = "cate_folder/" + name_of_this_element ;
  std::string name ;
...
//**************************************************************
std::vector< spectrum_abstr*>*  Tcate_telescope::cate_spectra_ptr()
{
...
void Tcate_telescope::analyse_current_event()
{
  ...
  {
    std::cout  <<  (par_force_segment_always_fired ?
  ...
  //============================================================
    ...
    spec_CsI_energy_raw->increment(CsI_energy_raw) ;
    spec_CsI_time_raw->increment(CsI_time_raw);
    ...
    )
  {
    ...
    spec_CsI_energy_fired->increment( CsI_energy_raw  ) ;

    //spec_CsI_energy->increment( 5) ;
    // incrementing the raw time spectrum
    spec_CsI_time_fired->increment(CsI_time_raw);
...
// Franco wants calibrated spectra now matter if time is OK or not, so below is commented
    ...
    spec_CsI_energy_cal->increment(CsI_energy_calibrated) ;
    spec_CsI_time_cal->increment(CsI_time_calibrated) ;
    owner->spec_total_CsI_time_cal->increment(CsI_time_calibrated) ;
    owner->CsI_time_calibrated = CsI_time_calibrated;
#ifdef CATE_PRESENT
    TRisingAnalysis::fxCaliEvent->cate_E_seg[segment_nr] = CsI_energy_calibrated;
#endif //CATE_PRESENT
  ...
  // the Si   part ============================================
  //============================================================
    spec_Si_energy_raw->increment( Si_energy_raw  ) ;
    spec_Si_time_raw->increment(Si_time_raw);
    ...
    )
  {
    ...
    spec_Si_energy_fired->increment( Si_energy_raw ) ;
    spec_Si_time_fired->increment(Si_time_raw);
  }
  ...
  spec_Si_energy_cal->increment(Si_energy_calibrated) ;
    TRisingAnalysis::fxCaliEvent->cate_dE_seg[segment_nr] = Si_energy_calibrated;
...
//     Si_time_calibrated >= 10 && Si_time_calibrated < 3800       // commented also
     ...
     )
  {
    ...
    spec_Si_time_cal->increment(Si_time_calibrated) ;
    owner->spec_total_Si_time_cal->increment(Si_time_calibrated) ;
  }
  ...
  spec_pin_raw[0]->increment(pin_raw[0] )  ;
  spec_pin_raw[1]->increment(pin_raw[1] )  ;
  spec_pin_raw[2]->increment(pin_raw[2] )  ;
  spec_pin_raw[3]->increment(pin_raw[3] )  ;
  ...
  for(int i = 0 ; flag_Si_segment_fired_good && (i < 4) ;i++)
  {
    if((pin_raw[i] > 250) &&  (pin_raw[i] < 4096))
    {
      ...
      pin_multipl++ ;
      spec_fan_signals_raw->increment(i) ;
    }
  }

  spec_fan_pin_mult_raw->increment(pin_multipl) ;

  // Fans spectra of Si
  if(Si_energy_raw )
    spec_fan_signals_raw->increment(4) ;
  if(Si_time_raw )
    spec_fan_signals_raw->increment(5) ;
  ...
  for(int i = 0 ; i < 4 ;i++)
  {
    ...
    spec_pin_cal[i]->increment(pin_cal[i]) ;  // if below is NIGDY
  }
  ...
  if(flag_all_pins_in_gates)
  {
    ...
    if(mianownik !=0)
    {
      ...
      matr_local_position_xy->increment(x, y);

      matr_local_position_xy_corr->increment(x_corr, y_corr);
    }
  }
  ...
  // this sould be only FIRED matrix
    matr_dE_E_fired ->increment( CsI_energy_calibrated,
                                    Si_energy_calibrated);
    ...
    if(flag_CsI_segment_fired_good  && flag_Si_segment_fired_good)
    {
      ...
      // OBSOLATE -----------
      matr_dE_E ->increment( CsI_energy_calibrated,
                                    Si_energy_calibrated);

      // New way
      matr_dE_E_good ->increment( CsI_energy_calibrated,
                                    Si_energy_calibrated);


      owner->matr_total_dE_E->increment(
...
void Tcate_telescope::make_preloop_action(std::ifstream & s)
{
  //
  //  std::cout << "preloop action for cate_telescope "
  //       << name_of_this_element
  //       << std::endl ;



  // std::cout << "Reading the calibration for the cate_telescope "
  //        << name_of_this_element
  //        << std::endl ;

  if(!event_cate_telescope_data)
  {
    std::cout << "Alarm, in " << name_of_this_element
         << " the pointer to event was not set" << std::endl;
    exit (1) ;
  }
  ...
  std::string slowo = name_of_this_element
  ...
  for(int i = 0 ; i < how_many_factors ; i++)
  {
    ...
    if(!s) {
      std::string mess = "I found keyword:" + slowo
  ...
  {
    std::string mess = "It should be at least 2 calibration factors "
  ...
  std::string name_of_pin ;

  for(int symb = 0 ; symb < 4 ; symb++)
  {
    std::ostringstream plik ; // (name_of_pin, sizeof(name_of_pin));
    ...
    for(int i = 0 ; i < how_many_factors ; i++)
    {
      ...
      pin_calibr_factors[symb].push_back(value ) ; break ;
...
// Y ------------------------------------------
  ...
  // std::cout << " reading the geometry angles  for cate_telescope "
  //        << name_of_this_element
  //        << std::endl ;
  ...
  {
    std::cerr << "Error while reading (inside geometry file "
         << geometry_file
...
void Tcate_telescope::read_gates()
{
  ...
  for(int symb = 0 ; symb < 4 ; symb++)
  {
    std::string core_of_the_name = path.dir_gates() + " + name_of_this_element ;
    std::string nam_gate = core_of_the_name + "_Si_pin_" ;
    nam_gate += (char)('a' +symb);
    nam_gate += "_raw_gate_GATE.gate" ;

    std::ifstream plik_gate(nam_gate.c_str() );

    if(! plik_gate)
    {
      std::string mess = "I can't open  file: " + nam_gate
        + "\nThis gate you can produce interactively using a 'grebware' viewer program" ;
      ...
      if(! plik_gate)
      {
        std::cout <<  "Error while reading 2 numbers from the gate file: "
          + nam_gate  ;
        exit(-1) ;
      }
      plik_gate.close() ;
...
//  std::cout << " Accepted " << ptr << std::endl;
