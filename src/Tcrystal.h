...
///////////////////////////////////////////////////////////////////////////////////////////////////////////
class Tcristal : public Tincrementer_donnor
{
public:
  static TEAGLE_array  *array_ge_pointer ;
  static Ttarget * target_rising_ptr ;    // for faster acces of the doppler correction
  static int multiplicity ;
  static double sum_energy_cal ;
  static double sum_energy_dop ;
...
protected:
  ...
  std::vector<double> energy_calibr_factors ;
  std::vector<double> energy20_calibr_factors ;
  std::vector<double> time_calibr_factors ;

#if CURRENT_EXPERIMENT_TYPE == G_FACTOR_OCTOBER_2005
  // calibration ffactors for common time singal TAC (Long range)
  std::vector<double> LRtime_calibr_factors ;
  double LRtime_calibrated;
  double time_cal_of_the_first_cristal_minus_time_cal;  // this one, which fired first
  double LR_plus_dt_time_cal ; // Sum of LRtime + above time
  bool flag_dt_calculated;
  bool flag_this_cristal_triggered_electronics;
  ...
  spectrum_1D * spec_time_cal_of_the_first_cristal_minus_time_cal;
...
public:
  ...
  static Tcristal *pointer_to_the_cristal_which_first_fired;
  static double time_of_the_cristal_which_first_fired;
  //  bool flag_I_was_the_cristal_which_started_the_trigger;
  double give_calibrating_offset()
  {
    if(LRtime_calibr_factors.size() > 0) return LRtime_calibr_factors[0];
    else
    {
      std::cout << "Tcristal.h  -  Error, empty vector of LRtime_calibr_factors" << std::endl;
      return 0;
    }
  }
...
protected:
  ...
  double energy_doppler_corrected_betaN[9] ;
...
public:
  Tcristal (Tcluster *parent_ptr, int nr) ;
  virtual ~Tcristal () {}  ;
...
/***********
  bool check_flag_good_fired()
  {
    ...
    {
      std::cout << name_of_this_element << ", flag_good_data = " << flag_good_data 
			<< ", flag_fired = " << flag_fired  << std::endl;
			return true;
    }
		return false;
  }
**********/
  ...
  void make_preloop_action(std::istream & );    // read the calibr. factors, gates
  std::string give_name() const { return name_of_this_element ;}
  double give_phi_geom()   const { return phi_geom ; }
  double give_theta_geom() const { return theta_geom ; }
  double give_x() const { return cristal_position_x ; }
  double give_y() const { return cristal_position_y ; }
  double give_z() const { return cristal_position_z ; }

  // for coincidence matrices
  double give_energy_raw() const { return energy_raw ; }
  double give_energy_calibrated() const { return energy_calibrated ; }
  double give_energy_doppler_corrected () const
    { return energy_doppler_corrected ; }
  double give_energy20_raw() const { return energy20_raw ; }
  double give_energy20_calibrated() const { return energy20_calibrated ; }
  double give_energy20_doppler_corrected () const
    { return energy20_doppler_corrected ; }
  double give_time_raw() const { return time_raw ; }
  double give_time_calibrated() const { return time_cal ; }
  ...
  /** Options are read from the disk by TRising class (during preloop)
      and then are stored in Tcristal static values */
  ...
  /** No descriptions */
  void increment_cristal_spectra_cal_and_doppler_when_mult_1();
