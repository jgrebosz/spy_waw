...
protected:
  ...
  // calibration factors
  std::vector<double> CsI_energy_calibr_factors;
  std::vector<double> CsI_time_calibr_factors;
  std::vector<double> Si_energy_calibr_factors;
  std::vector<double> Si_time_calibr_factors;

  std::vector< std::vector<double> > pin_calibr_factors;  // [4][3]

  std::vector<double>  x_position_corr_factors;
  std::vector<double>  y_position_corr_factors;
  ...
  double low_g[4] ;   // have to be double, bec grebware stores value of
  // gates as double
  double high_g[4] ;

  // Cate data structure, defined on the top of the TRisingEvnet.h file
  /*-----------------------
    struct cate_segment
    {
    int CsI_energy,
...
public:
  ...
  Tcate_telescope(Tcate *ptr_owner, std::string name, int id_nr);
  virtual ~Tcate_telescope();

  // Cate register of all spectra
  std::vector< spectrum_abstr*>*  cate_spectra_ptr();
...
//   void set_event_data_pointers(cate_segment * ptr);
  ...
  double give_central_x() { return central_x ; }
  double give_central_y() { return central_y ; }
  double give_x() { return central_x + x; }
  double give_y() { return central_y + y; }
  double give_local_x() { return x; }
  double give_local_y() { return y; }

  //// for corrected position
  double give_x_corr() { return central_x + x_corr; }
  double give_y_corr() { return central_y + y_corr; }
  double give_local_x_corr() { return x_corr; }
  double give_local_y_corr() { return y_corr; }


  /** If position was calculated precisely */
  bool was_xy_ok() { return flag_xy_ok ; }
  ...
  double give_dE() {return Si_energy_calibrated  ;}
  double give_E() { return CsI_energy_calibrated ; }
  double give_dE_raw() {return Si_energy_raw  ;}
  double give_E_raw() { return CsI_energy_raw ; }
