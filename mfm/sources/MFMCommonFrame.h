...
virtual void SetDataSource(uint8_t datasource);
virtual void SetRevision(uint8_t  revision);
virtual void SetFrameSize(uint32_t  size);
virtual void SetFrameType(uint16_t frametype);
...
virtual int  jurek_ReadInFile(ifstream &file,char** vector, int * vectosize);
virtual int  FillBigBufferFromFile(int fLun,char* vector, unsigned int  vectosize,unsigned int *readsize,unsigned int *eventcount);
...
virtual bool IsABlobType(int type =0);
virtual void FillStat();
virtual void InitStat();
virtual void IncrementNegativJump();
virtual void IncrementNoContiJump();
